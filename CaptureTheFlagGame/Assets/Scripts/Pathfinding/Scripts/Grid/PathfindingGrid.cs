...
namespace Assets.Scripts.Pathfinding.Scripts.Grid
{
    public class PathfindingGrid : MonoBehaviour
    {
        ...
        private void CreateGrid()
        {
            ...
            for (var x = 0; x < _gridSizeX; x++)
            {
                for (var y = 0; y < _gridSizeY; y++)
                {
                    ...
                    //var hitColliders = Physics.OverlapSphere(worldPoint, NodeRadius);

                    var walkable = Physics.CheckSphere(worldPoint, NodeRadius, WalkableMask);
                    //walkable = hitColliders.All(c => WalkableMask == (WalkableMask | (1 << c.gameObject.layer))) && walkable;
                    ...
                    if (Physics.Raycast(ray, out var hit, 10000, _walkableMask))
        ...
        //            var ray = new Ray(worldPoint + Vector3.up * 50, Vector3.down);
        //            RaycastHit hit;
        //            if (Physics.Raycast(ray, out hit, 10000, _walkableMask))
        //                _walkableRegionsDictionary.TryGetValue(hit.collider.gameObject.layer, out movementPenalty);
        ...
        /// <summary>
        /// Updates the maximum and minimal penalties
        /// </summary>
